...
Wave buoy datasets are configured in a single YAML file in `projects/wis2/` (`WAVE_BUOYS.yaml`). The file defines the shared `minio_path` and a list of targets, each matching an S3 path prefix to a WIGOS ID.
...
-   `targets`: The list of buoy targets.
-   `targets[].name`: Friendly name used in logs.
-   `targets[].match_on.prefix`: S3 path prefix used to match incoming files.
-   `targets[].wigos_id`: The WIGOS station identifier for the buoy, used in the output CSV file.

The schema is validated by `projects/wis2/config_schema.py`; the flow resolves the WIGOS ID by prefix matching and logs a warning if no target matches the incoming path.
...
```bash
aws s3 cp projects/wis2/sample/IMOS_COASTAL-WAVE-BUOYS_20250801_APOLLO-BAY_RT_WAVE-PARAMETERS_monthly.nc s3://imos-data-nonproduction/IMOS/COASTAL-WAVE-BUOYS/WAVE-BUOYS/REALTIME/WAVE-PARAMETERS/APOLLO-BAY/2025/IMOS_COASTAL-WAVE-BUOYS_20250801_APOLLO-BAY_RT_WAVE-PARAMETERS_monthly.nc
```
