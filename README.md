...
1. **Weather Service Office, Chuuk** (Land-based)
   - WIGOS ID: 0-20000-0-91334
   - Location: 7.4539°N, 151.8425°E

2. **Apollo Bay Wave Buoy** (Sea-based)
   - WIGOS ID: 0-22000-0-7811080  
   - Location: 38.7541°S, 143.7232°E

> **Note**: The Beni-Abbes station entry was removed from the station list as part of recent configuration updates.
...
- **Temporal Resolution**: 30 minutes (PT30M)
...
| `WPDI` | `directionFromWhichDominantWavesAreComing` | Dominant wave direction |
| `WPFM` | `averageWavePeriod` | Average wave period |
| `WPPE` | `spectralPeakWavePeriod` | Peak wave period |
| `WSSH` | `significantWaveHeight` | Significant wave height |
...
2. **BUFR to GeoJSON**: Converts BUFR messages to GeoJSON for web services
...
- **Data Source**: Coastal wave buoys providing real-time observations
