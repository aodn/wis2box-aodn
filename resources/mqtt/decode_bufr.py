...
def decode_section4(handle: int) -> list[str]:
    """Decode section 4 data values using the expanded descriptor key names."""
    lines = [
        "",
        "=" * 70,
    ...
    seen_keys: dict[str, int] = {}  # track duplicate key names → occurrence index

    for abbr, unit, name in zip(abbrevs, units, names):
        # Skip structural/operator descriptor pseudo-keys that have no data value
        if abbr in ("", None) or str(abbr).isdigit():
            continue
        ...
        try:
            if count == 0:
                raw = eccodes.codes_get(handle, key)
            else:
                # eccodes appends #N# prefix for repeated keys (1-based N)
                raw = eccodes.codes_get(handle, f"#{count + 1}#{key}")
        ...
        lines.append(f"    {key:<65}  {value_str:<20}  {unit}")
